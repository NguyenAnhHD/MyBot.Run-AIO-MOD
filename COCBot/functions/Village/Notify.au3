...
Func NotifyGetLastMessageFromTelegramBtnStart()
	If $g_iDebugSetlog Then SetDebugLog("Notify | NotifyGetLastMessageFromTelegramBtnStart()")
	...
	If _IsInternet() < 1 Then
	     Setlog("Telegram: Check your internet connection! No Connection..", $COLOR_RED)
		 Return
	EndIf
...
Func NotifyRemoteControlProcBtnStart()
	If $g_bNotifyTGEnable = True And $g_sNotifyTGToken <> "" Then
		...
		If $g_iDebugSetlog Then Setlog("Telegram | NotifyRemoteControlProcBtnStart $TGActionMSG : " & $TGActionMSG)
		If $g_iDebugSetlog Then Setlog("Telegram | NotifyRemoteControlProcBtnStart $g_iTGLastRemote : " & $g_iTGLastRemote)
		If $g_iDebugSetlog Then Setlog("Telegram | NotifyRemoteControlProcBtnStart $g_sTGLast_UID : " & $g_sTGLast_UID)
...
Func NotifyRemoteControlProc()
	...
	;Telegram ---------------------------------------------------------------------------------
	If $g_bNotifyTGEnable = True And $g_sNotifyTGToken <> "" And $g_bRunState = True Then
		...
		Else
			If $g_iTGLastRemote <> $g_sTGLast_UID Then
				...
				Switch $TGActionMSG
					...
					Case GetTranslatedFileIni("MBR Func_Notify", "START", "START"), '\u25b6 ' & GetTranslatedFileIni("MBR Func_Notify", "START", "START")
						If $g_bRunState = True Then
							SetLog("Notify Telegram" & ": " & "Your bot is currently started, no action was taken", $COLOR_GREEN)
