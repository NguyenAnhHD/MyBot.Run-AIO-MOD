	...
	Switch $Message
		...
		Case "LastRaid"
			If $g_bNotifyAlerLastRaidTXT Then
				...
				NotifyPushToTelegram($g_sNotifyOrigin & " | " & GetTranslatedFileIni("MBR Func_Notify", "Last-Raid_Info_04", "Last Raid txt") & _
				"%0A" & "[" & "G" & "]: " & _NumberFormat($g_iStatsLastAttack[$eLootGold]) & _
				"k  [" & "E" & "]: " & _NumberFormat($g_iStatsLastAttack[$eLootElixir]) & _
				"k  [" & "DE" & "]: " & _NumberFormat($g_iStatsLastAttack[$eLootDarkElixir]) & _
				"k %0A[" & "T" & "]: " & $g_iStatsLastAttack[$eLootTrophy] & _
				"  [" & "%" & "]: " & $g_sTotalDamage & _
				"  [" & "*" & "]: " & $g_sStarsEarned & _
