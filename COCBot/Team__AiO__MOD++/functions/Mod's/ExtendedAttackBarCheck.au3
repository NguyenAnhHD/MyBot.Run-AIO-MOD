...
Func ExtendedAttackBarCheck($aTroop1stPage, $Remaining)
	...
	If IsArray($res) Then
		...
		Else
			...
			If $g_bDebugSetlog Then SetDebugLog("$res[0] = " & $res[0])
			...
			For $i = 0 To UBound($aKeys) - 1
				...
				Else
					...
					$aCoordArray[0][1] = -1
				EndIf
				If $g_bDebugSetlog Then SetDebugLog($aResult[$i][0] & " | $aCoordArray: " & $aCoordArray[0][0] & "-" & $aCoordArray[0][1])
				;;;;;;;; If exist Castle Spell ;;;;;;; - Fix case 2 CC Spells (Demen)
				If UBound($aCoords) > 1 And StringInStr($aResult[$i][0], "Spell") <> 0 Then
					If $g_bDebugSetlog Then SetDebugLog($aResult[$i][0] & " x" & UBound($aCoords) & " multiple detected: " & $aValue)
			...
			For $i = 0 To UBound($aResult) - 1
				...
				If $aResult[$i][1] > 0 Then
					...
					If $iDublicateSlot <> -1 Then
						If $g_bDebugSetlog Then SetDebugLog($aResult[$i][0] & " is already found in 1st page at Slot: " & $aTroop1stPage[$iDublicateSlot][1])
						ContinueLoop
					EndIf
			...
			If Not $Remaining Then
				...
				If $g_bDebugSetlog Then SetDebugLog("$iTotalSlot1stPage : " & $iTotalSlot1stPage, $COLOR_DEBUG) ;Debug
				$g_iTotalAttackSlot = $iSlotExtended + $iTotalSlot1stPage + $iCCSpell
			EndIf

			If $g_bDebugSetlog Then SetDebugLog("$iSlotExtended / $iCCSpell / $g_iTotalAttackSlot: " & $iSlotExtended & "/" & $iCCSpell & "/" & $g_iTotalAttackSlot)

		EndIf
	EndIf

	If $g_bDebugSetlog Then SetDebugLog("Extended String: " & $strinToReturn)
...
Func DragAttackBar($iTotalSlot = 20, $bBack = False)
	...
	If Not $bBack Then
		If $g_bDebugSetlog Then SetDebugLog("Dragging attack troop bar to 2nd page. Distance = " & $iTotalSlot - 9 & " slots")
		ClickDrag(25 + 73 * ($iTotalSlot - 9), 660, 25, 660, 1000)
		If _Sleep(1000 + $iTotalSlot * 25) Then Return
		$bAlreadyDrag = True
	Else
		If $g_bDebugSetlog Then SetDebugLog("Dragging attack troop bar back to 1st page. Distance = " & $iTotalSlot - 9 & " slots")
